...
def _train_distances_no_replay(
                    model,
                    rewards:dict,
                    epochs:int,
                    optimizer,
    ...
    for e in range(epochs):
        ...
        for t in tqdm(range(n_frames)):
            ...
            prev_state = {"screen" : torch.stack([w[:, t], bird_mask.float()], dim=1),
                        "vy" : game.flappy.vy.clone()}
            
            Q_actions = model(prev_state)
            
            if torch.rand(1).item() < epsilon_current:
            ...
            state = {"screen" : torch.stack([game.step(game.t), bird_mask.float()], dim=1),
                    "vy" : game.flappy.vy.clone()}
