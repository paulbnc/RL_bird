...
class model_position(nn.Module):
    ...
    def forward(self, obs):
        state = obs["screen"]
        vy  = obs["vy"]
        features = self.get_distance(state, vy)
        x = self.f3(self.act(self.f2(self.act(self.f1(features)))))
        return x
    

    def get_distance(self, state, vy):
        #on calcule 3 features de distance
        #i)bord droit oiseau - bord gauche tuyau
        #ii)centre oiseau - haut du trou
        #iii)centre oiseau - bas du trou
        ...
        for i in range(B):
            ...
            bird_cols = bird.any(dim=0).nonzero(as_tuple=True)[0]
            bird_rows = bird.any(dim=1).nonzero(as_tuple=True)[0]
    
            if bird_cols.numel() == 0 or bird_rows.numel() == 0:
                continue
    
            bird_x_right  = int(bird_cols.max().item())            
            bird_center_y = float(bird_rows.float().mean().item()) 
